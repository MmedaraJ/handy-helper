        ...
        if len(post_data['signin_password']) < 1:
            ...
            if len(user)>0:
                if not bcrypt.checkpw(post_data['signin_password'].encode(), user[0].password_hash.encode()): 
                    errors['signin_pw'] = "Incorrect password"
        return errors
